    ...
    <section class="content-header">
        <h1>@lang($module_name)
            <a class="btn btn-sm btn-warning pull-right" href="{{ route(Request::segment(2) . '.create') }}"><i
                    class="fa fa-plus"></i> @lang('Thêm mới đơn xin nghỉ phép')</a>
        </h1>
    </section>
@endsection
    ...
    <section class="content">
        ...
        <div class="box box-default">
            ...
            {{-- <div class="d-flex align-items-center gap-2" style="margin: 21px">
                <form action="{{ route('salary_payment.import') }}" method="POST" enctype="multipart/form-data"
                        ...
                        <table class="table table-hover table-bordered">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-center">@lang('Số ngày được nghỉ phép')</th>
                                    <th class="text-center">@lang('Số ngày đã sử dụng')</th>
                                    <th class="text-center">@lang('Số ngày còn lại')</th>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <a class="btn btn-sm btn-primary"
                                                href="{{ route('leave_balance.show', $row->id) }}" data-toggle="tooltip"
                                            ...
                                            <a class="btn btn-sm btn-warning" data-toggle="tooltip"
                                                ...
                                                href="{{ route('leave_balance.edit', $row->id) }}">
                                                <i class="fa fa-pencil-square-o"></i>
                                            </a>
                                            <form action="{{ route('leave_balance.delete', $row->id) }}" method="POST"
...
{{-- @section('script')
    <script>
        ...
        $(document).ready(function() {
            ...
            // Save salary_payment
            $('.btn_salary_payment').click(function() {
                var studentId = $(this).data('id');
                var classId = $('.class_id').val();
                var trackedAt = $('.tracked_at').val();
                var status = $('input[name="salary_payment[' + studentId + '][status]"]:checked').val();
                var parentId = $('select[name="salary_payment[' + studentId + '][checkin_parent_id]"]').val();
                var teacherId = $('select[name="salary_payment[' + studentId + '][checkin_teacher_id]"]').val();
                var note = $('input[name="salary_payment[' + studentId + '][json_params][note]"]').val();
                var img = $('input[name="salary_payment[' + studentId + '][json_params][img]"]').val();
                ...
                $.ajax({
                    url: '{{ route('salary_payment.store') }}',
                    type: 'POST',
                    data: {
